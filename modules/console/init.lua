...
path = ...
--error(...)
path_req = path--path:sub(1, -9)
--error(path_req)
path_load = path:gsub("%.", "/") --path:sub(1, -9):gsub("%.", "/")
--error(path_load)
...
function update_ui(w, h)
    ...
    local h = h --h / 3
    ui.console_canvas_pos = {x = 0, y = h/2, w = w, h = h}
    ui.background_canvas = love.graphics.newCanvas()
    ui.text_canvas = love.graphics.newCanvas()
	ui.background = {x = 0, y = 0, w = w, h = ui.console_canvas_pos.h - ui.console_canvas_pos.y , color = background_color}
...
function draw_ui()
    ...
    love.graphics.setCanvas(ui.background_canvas)
    love.graphics.clear( )
	love.graphics.setColor(ui.background.color) --ui.background.color
	love.graphics.rectangle("fill", ui.background.x, ui.background.y, ui.background.w, ui.background.h + font_h)

    --love.graphics.setCanvas(ui.text_canvas)
    ...
    love.graphics.draw(ui.background_canvas, ui.console_canvas_pos.x, ui.console_canvas_pos.y)
    love.graphics.setColor(util.to_rgb_table("#FFFFFFFF"))
end
