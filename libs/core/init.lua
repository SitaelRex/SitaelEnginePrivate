...
function Module.new(_,ownerCore, name, path)
    ...
    local moduleCallable = getmetatable(resultModule.source) and getmetatable(resultModule.source).__call or nil
    local moduleMtt = {--__index =  resultModule.source
...
local function LoadModule(self,moduleName,curModulePath)
    ...
    self.modules[moduleName] = self.modules[moduleName] or Module.new(_,moduleOwner, moduleName,curModulePath)-- require(curModulePath) -- если модуль ещё не загружен
end;
...
local function ConnectInterface(self,inerfaceName, moduleName)
end;

local function DisconnectInterface(self,inerfaceName)
end;
