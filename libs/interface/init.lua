...
local GetProxy = function(interface) return proxyStorage[interface] end;

local SetProxy = function(interface, proxy) proxyStorage[interface] = proxy end;
-----------------------------------public
...
function Interface:Disconnect() end
...
function Interface:RemoveMethod(name) self.methods[name] = nil end
...
function Interface.new(_, ownerCore, name)
    ...
    setmetatable(resultInterface, {
        __call = function(self, ...)
            local tableConnected = resultProxy.mod and
                                       type(resultProxy.mod.source) == "table"
            local functionConnected = resultProxy.mod and
                                          type(resultProxy.mod.source) ==
                                          "function"

            local moduleCall = tableConnected and
                                   getmetatable(resultProxy.mod.source).__call or
                                   (functionConnected and resultProxy.mod.source)
            return moduleCall(...)
        end,
        __index = function(_, k)
            ...
            if type(result) == "function" then
                local aresult = function(maybeSelf, ...)
                    local isSelfCall = type(maybeSelf) == "table" and
                                           maybeSelf.isInterface
...
return setmetatable(Interface,
                    {__call = function(...) return Interface.new(...) end})
