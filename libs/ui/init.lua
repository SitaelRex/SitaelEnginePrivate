...
local function SetFileManagerOpen(status) openManager = status end
local function SetProjectCreatorOpen(status) openProjectsCreator = status end
local function SetProjectNamingOpen(status) openProjectNaming = status end
...
local function LinkCore(Core) -- генерирует интерфейсы и их связи из linkConfig.txt
    ...
    local function LinkModule(line)
        ...
        Interface.Connect(interfacesConnect[interfaceName],
                          Core:IndexModule(moduleName))
    end
...
local function OpenProject(projectName)
    ...
    if not errormsg then
        ProjectCore = nil
        ProjectCore = Core(EngineCore,
                           EngineCore.modules.pathManager.GetProjectPath())
        setmetatable(_G, {
        ...
        setmetatable(_G, {
            __index = function(_, key)
                ...
                if not sourceModule then
                    ...
                    if sourceInfo == key or
                        (included[sourceInfo] and included[sourceInfo][key]) then
                        return ProjectCore:IndexInterface(key)
                    end
                end
            end
    ...
    for _, name in pairs(templateFiles) do
        ...
        if info then
            if info.type == "file" then
                love.filesystem.write(path .. "/" .. name, love.filesystem
                                          .read(templatePath .. "/" .. name))
            elseif info.type == "directory" then
                love.filesystem.createDirectory(path .. "/" .. name)
                RecursiveCopy(templatePath .. "/" .. name, path .. "/" .. name,
                              projectType)
            end
        end
...
local function OpenProjectNaming(projectType)
    if openProjectNaming then
        ...
        if imgui.BeginPopupModal("ProjectNaming", nil,
            ...
            if imgui.Button("cancel", 80, 0) then
                openProjectNaming = false;
                imgui.CloseCurrentPopup()
            end
            imgui.SameLine()
            if imgui.Button("open", 80, 0) then
                openProjectNaming = false;
                openProjectsCreator = fasle;
                imgui.CloseCurrentPopup();
                CreateProject(str, projectType)
            end
            imgui.EndPopup();
...
local function OpenProjectCreator()
    if openProjectsCreator then
        ...
        local open, close = imgui.BeginPopupModal("ProjectCreate", nil,
...
local function OpenFileManger()
    if openManager then
        ...
        if imgui.BeginPopupModal("ProjectSelect", nil, {
            "ImGuiWindowFlags_NoResize", "ImGuiWindowFlags_NoClose"
        }) then
            imgui.Text("выбери проект")
            imgui.Separator();
            local projectsList = love.filesystem.getDirectoryItems("projects") -- "projects"
            local _
            _, selected = imgui.ListBox("", selected, projectsList,
                                        #projectsList, 4) -- ,1,love.filesystem.getDirectoryItems( "projects" ),3)
            imgui.Separator();
            if imgui.Button("cancel", 80, 0) then
                openManager = false;
                imgui.CloseCurrentPopup()
            ...
            if imgui.Button("open", 80, 0) then
                openManager = false;
                imgui.CloseCurrentPopup();
                OpenProject(projectsList[selected])
            end
            imgui.EndPopup();
...
local function draw()
    ...
    if imgui.BeginMainMenuBar() then
        if imgui.BeginMenu("Engine") then imgui.EndMenu() end
        if imgui.BeginMenu("Project") then
            if imgui.MenuItem("New") then SetProjectCreatorOpen(true) end
            if imgui.MenuItem("Open") then SetFileManagerOpen(true) end
            if imgui.MenuItem("Close", nil, false, true) then -- label,  shortcut, bool selected, bool enabled
                ProjectCore = Core(EngineCore,
                                   EngineCore.modules.pathManager.projectPath) -- доступ к project core
            end
            imgui.EndMenu()
        end
        if imgui.BeginMenu("Settings") then imgui.EndMenu() end
        ...
        if imgui.Checkbox("Show Demo", sdw) then sdw = not sdw end
    ...
    OpenFileManger()
    OpenProjectCreator()
    if sdw then showTestWindow = imgui.ShowDemoWindow(false) end
    IMAGE = canvas
    imgui.SetNextWindowPos(0, 10)
    imgui.SetNextWindowSize(love.graphics.getWidth(),
...
InterfaceUnit.new = function(parent, interfaceFunc, funcArgs, funcBody,
                             interfaceTag) -- parent, imgui.something, args, function
    -- print(...)
    local result = {}
    ...
    if parentUnit then parentUnit.pool[result] = result end
...
InterfaceUnit.destroy = function(unit)
    ...
    if interfaceTag then interfaceUnitTags[interfaceTag][unit] = nil end

    unit = nil;
end;

local function emptyUnit() return true end
...
InterfaceUnit.log = function(self)
    local function printUnit(unit, level)
        ...
        for key, unit in pairs(unit.pool) do printUnit(unit, level + 1) end
    end
    printUnit(self, 0)
end

local InterfaceUnitMT = {
    __call = function(self, ...) return InterfaceUnit.new(...) end
}
