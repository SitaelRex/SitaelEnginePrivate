...
function World.new()
   local world = setmetatable({
      ...
      __updated = List(), __backUpdated = List(),
...
function World:updatePoolLayer(e)
    if not Type.isEntity(e) then
      error("bad argument #1 to 'World:updatePoolPosition' (Entity expected, got "..type(e)..")", 2)
   end
   
   self.__updated:add(e)
...
function World:__flush()
   -- Early out
   if (self.__added.size == 0 and self.__removed.size == 0 and self.__dirty.size == 0 and self.__updated.size == 0) then
      return self
   end
   ...
   self.__updated, self.__backUpdated = self.__backUpdated, self.__updated
   ...
   self.__backAdded:clear()
    ...
    if self.__backUpdated.size > 0 then
        engineEvent:EmitOnce("SystemPoolUpdated",self)
    end
    --for i =  1 , self.__backUpdated.size do
    --    e = self.__backUpdated[i]
    --    print("#####",e.hierarchy.object_name)
    --    if e.__world == self then
    --        -- print("updatePool")
    --        for j = 1, self.__systems.size do
    --            self.__systems[j]:__updatePool(e)
    --            -- print(j)
    --        end
    --        
    --       -- e.hierarchy.level =  e.hierarchy.new_level
    --       -- print("e.hierarchy.level",e.hierarchy.level)
    --        --e.hierarchy.new_level = nil
    --    end
    --end
   
   --for i = 1, self.__backUpdated.size do
   --    e = self.__backUpdated[i]
   -- e.hierarchy.old_level = 1
   --end
    self.__backUpdated:clear()
...
function World:onEntityRemoved(e) -- luacheck: ignore
  --e.onEntityRemoved = e.onEntityRemoved and e:onEntityRemoved()
  ecs.OnEntityDestroy(e)
  --local components = e:getComponents()
  --for d,_ in pairs(components) do
