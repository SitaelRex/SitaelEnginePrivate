...
local ComponentInherit = require(PATH..".componentInherit")
...
local function give(e, name, componentClass, ...)
   local component = componentClass:__initialize(...)
    ...
    local saveBlackList = {__isComponent = true,__componentClass = true,__isComponentClass = true,owner_object = true }
    --эти переменные содержат таблицы, через которые движок будет пытаться сохранить вобще все состояние игры
    --они не должны сохраняться ни в коем случае
    local serializableFields = {}
    for k,v in pairs(component) do
        if not saveBlackList[k] then
           -- print(k,v)
            serializableFields[k] = true
        end;
    end;
    serializableFields["__serializableFields"] = true
    component.__serializableFields = serializableFields
    e[name] = component
...
function Entity:updatePoolLayer()
  --  print("enity",self)
    if self.__world then
      self.__world:updatePoolLayer(self)
   end
    return self
end;
...
function Entity:serialize()
  ...
  -- data.__name = self.obj_type or "UNDEFINED_ENTITY"
   return data
end
...
function Entity:GetAbsolute(component)
   return ComponentInherit.GetAbsolute(self,component)
end;
